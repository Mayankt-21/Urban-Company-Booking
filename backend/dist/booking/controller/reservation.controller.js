"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.ReservationController = void 0;
const create_reservation_dto_1 = require("../dto/create-reservation.dto");
const booking_gateway_1 = require("../gateway/booking.gateway");
const reservation_service_1 = require("./../service/reservation.service");
const common_1 = require("@nestjs/common");
let ReservationController = class ReservationController {
    constructor(reservationService, bookingGateway) {
        this.reservationService = reservationService;
        this.bookingGateway = bookingGateway;
    }
    async getAllReservations() {
        return this.reservationService.getAllReservations();
    }
    async bookSlot(createReservationDto) {
        const reservation = await this.reservationService.bookSlot(createReservationDto);
        this.bookingGateway.notifySlotBooked(reservation.slot.id, reservation.userName);
        return reservation;
    }
    async cancelReservation(id) {
        const response = await this.reservationService.cancelReservation(id);
        this.bookingGateway.notifySlotCanceled(id);
        return response;
    }
};
exports.ReservationController = ReservationController;
__decorate([
    (0, common_1.Get)(),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", []),
    __metadata("design:returntype", Promise)
], ReservationController.prototype, "getAllReservations", null);
__decorate([
    (0, common_1.Post)(),
    __param(0, (0, common_1.Body)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [create_reservation_dto_1.CreateReservationDto]),
    __metadata("design:returntype", Promise)
], ReservationController.prototype, "bookSlot", null);
__decorate([
    (0, common_1.Delete)(':id'),
    __param(0, (0, common_1.Param)('id')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Number]),
    __metadata("design:returntype", Promise)
], ReservationController.prototype, "cancelReservation", null);
exports.ReservationController = ReservationController = __decorate([
    (0, common_1.Controller)('reservation'),
    __metadata("design:paramtypes", [reservation_service_1.ReservationService,
        booking_gateway_1.BookingGateway])
], ReservationController);
//# sourceMappingURL=reservation.controller.js.map